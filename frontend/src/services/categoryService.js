...
class CategoryService extends BaseService {
  ...
  async getCategoryListingCount(categoryId) {
    try {
      ...
      if (typeof response === "object" && response !== null) {
        ...
        // If response has data array, use its length as fallback
        if (count === 0 && Array.isArray(response.data)) {
  ...
  /**
   * Refresh listing counts for multiple categories
   * @param {Array} categoryIds - Array of category IDs to refresh
   * @returns {Promise<Object>} Object with categoryId as key and count as value
   */
  async refreshCategoryListingCounts(categoryIds) {
    try {
      console.log(`🔄 Refreshing listing counts for categories:`, categoryIds);
      ...
      const countPromises = categoryIds.map(async (categoryId) => {
        try {
          ...
          console.error(
            `Error refreshing count for category ${categoryId}:`,
            error
          );
      ...
      console.log(`✅ Refreshed category counts:`, counts);
      return counts;
    } catch (error) {
      console.error("Error refreshing category listing counts:", error);
      return {};
    }
  }
