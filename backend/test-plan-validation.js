...
async function testPlanValidation() {
  ...
  try {
    ...
    const featuredLimits1 = getPlanFeaturedLimits(testPlan1);
    const featuredLimits2 = getPlanFeaturedLimits(testPlan2);

    console.log("Plan 1 listing limits:", limits1);
    console.log("Plan 1 featured limits:", featuredLimits1);
    console.log("Plan 2 listing limits:", limits2);
    console.log("Plan 2 featured limits:", featuredLimits2);
    console.log(
      "✅ getPlanListingLimits and getPlanFeaturedLimits tests passed\n"
    );
    ...
    if (users && users.length > 0) {
      ...
      console.log(
        `📋 Test 4: Testing plan validation for vendor: ${testVendor.name}`
      );
      ...
      const validationResult = await validateListingCreation(
        testVendor.id,
        false
      );
      console.log("Listing creation validation (regular):", validationResult);

      const featuredValidationResult = await validateListingCreation(
        testVendor.id,
        true
      );
      console.log(
        "Listing creation validation (featured):",
